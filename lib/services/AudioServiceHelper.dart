...
class AudioServiceHelper {
  ...
  // final _audioPlayerHandler = GetIt.instance<AudioPlayer>();
  final audioServiceHelperLogger = Logger("AudioServiceHelper");
  final _downloadsHelper = GetIt.instance<DownloadsHelper>();
  final _jellyfinApiData = GetIt.instance<JellyfinApiData>();
  // final Box<List<BaseItemDto>> _baseItemBox = Hive.box("baseItem");
  final Box<BaseItemDto> _baseItemBox = Hive.box("baseItem");
  final _boxInitApp = Hive.box('initApp');
  ...
  Future<void> replaceQueueWithItem({
    ...
    try {
      if (initialIndex > itemList.length) {
        return Future.error("startAtIndex is bigger than the itemList! ($initialIndex > ${itemList.length})");
      }
      ...
      _boxInitApp.put('initialIndex', initialIndex);
      _baseItemBox.put('trackItem', itemList[initialIndex]);
      // _baseItemBox.put('trackItem', itemList);
  ...
  Future<void> queueWithItem({
    ...
    try {
      ...
      // _audioHandler.play();
      _audioHandler.seek2(Duration(seconds: seek));
      // _audioHandler.setPlaybackRate(speed);

      // baseItemBox.put('trackItem', itemList[initialIndex]);
    } catch (e) {
      audioServiceHelperLogger.severe(e);
      return Future.error(e);
    }
  ...
  Future<void> shuffleAll(bool isFavourite) async {
    ...
    if (FinampSettingsHelper.finampSettings.isOffline) {
      ...
      items = await _jellyfinApiData.getItemsMusic(
        isGenres: false,
  ...
  Future<MediaItem> _generateMediaItem(BaseItemDto item) async {
    ...
    return MediaItem(
      ...
      duration: Duration(
        microseconds: (item.runTimeTicks == null ? 0 : item.runTimeTicks! ~/ 10),
      ),
    );
  }
}
